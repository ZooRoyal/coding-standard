...
function show_help()
{
    ...
    echo "    -t <git tree-ish>    Finds PHP-Files which have changed since the current branch parted from the target branch only."
...
function find_diff_only()
{
    ...
    localBranch=${branchRef##refs/heads/}
    refinedDiff=()
...
function compute_blacklisted_directories()
{
    ...
    rawExcludePathsByFileByGit=$(find . -name .git -mindepth 2)
